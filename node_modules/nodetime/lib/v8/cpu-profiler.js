...
CpuProfiler.prototype.startCpuProfiler = function(seconds) {
  ...
  if(!self.v8tools) {
    return sendError("v8tools package was not loaded. Please make sure it is properly installed.");
  }

  if(self.active) {
    return sendError("CPU or heap profiler is already active.");
  }
...
CpuProfiler.prototype.stopCpuProfiler = function() {
  ...
  if(!self.v8tools || !self.active) return;

  var nodes = {};
  var root = undefined;
  var rootSamplesCount = undefined;

  self.v8tools.stopV8Profiler(function(parentCallUid, callUid, totalSamplesCount, functionName, scriptResourceName, lineNumber) {
